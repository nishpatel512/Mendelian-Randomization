...
def mv_format_data(dat, type="exposure", snps=None, header=True,
    ...
    # if other_allele_col in dat.columns:
        ...
        check_allele_columns(dat,effect_allele_col,other_allele_col)
...
def check_allele_columns(dat, effect_allele_col, other_allele_col):
    # Check effect_allele
    i = np.where(dat.columns == effect_allele_col)[0][0]
    if not np.isnan(i):
        dat.rename(columns={effect_allele_col: "effect_allele.outcome"}, inplace=True)
        if isinstance(dat["effect_allele.outcome"], pd.Series):
            dat["effect_allele.outcome"] = dat["effect_allele.outcome"].astype(str).str[0]
        if not isinstance(dat["effect_allele.outcome"], str):
            print("effect_allele column is not character data. Coercing...")
            dat["effect_allele.outcome"] = dat["effect_allele.outcome"].astype(str)

        dat["effect_allele.outcome"] = dat["effect_allele.outcome"].str.upper()
        index = ~((dat["effect_allele.outcome"].str.match("^[ACTG]+$")) | (dat["effect_allele.outcome"].isin(["D", "I"])))
        index.loc[index.isna()] = True
        if index.any():
            ...
            dat.loc[index, "effect_allele.outcome"] = np.nan
            dat.loc[index, "mr_keep.outcome"] = False

    # Check other_allele
    i = np.where(dat.columns == other_allele_col)[0][0]
    if not np.isnan(i):
        dat.rename(columns={other_allele_col: "other_allele.outcome"}, inplace=True)
        if isinstance(dat["other_allele.outcome"], pd.Series):
            dat["other_allele.outcome"] = dat["other_allele.outcome"].astype(str).str[0]
        if not isinstance(dat["other_allele.outcome"], str):
            print("other_allele column is not character data. Coercing...")
            dat["other_allele.outcome"] = dat["other_allele.outcome"].astype(str)

        dat["other_allele.outcome"] = dat["other_allele.outcome"].str.upper()
        index = ~((dat["other_allele.outcome"].str.match("^[ACTG]+$")) | (dat["other_allele.outcome"].isin(["D", "I"])))
        index.loc[index.isna()] = True
        if index.any():
            print("other_allele column has some values that are not A/C/T/G or an indel comprising only these characters or D/I. These SNPs will be excluded.")
            dat.loc[index, "other_allele.outcome"] = np.nan
            dat.loc[index, "mr_keep.outcome"] = False
